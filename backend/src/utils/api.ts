...
if (!process.env?.EXPO_PUBLIC_API_URL) {
	console.warn(
		"EXPO_PUBLIC_API_URL not set; defaulting to http://localhost:4000",
	);
}
...
interface DetectProductApiItem {
...
interface DetectProductsApiResponse {
	detected?: DetectProductApiItem[];
	image_path?: string | null;
	path?: string | null;
}

/**
 * Uploads an image and requests product detection from backend.
 */
export async function detectProductsFromImage(
	uri: string,
): Promise<{ detected: BillItem[]; path: string | null }> {
	const token = await getToken();
	const formData = new FormData();
	...
	const data = await handleResponse<DetectProductsApiResponse>(res);
	const detected = (data.detected ?? []).map((item) => ({
